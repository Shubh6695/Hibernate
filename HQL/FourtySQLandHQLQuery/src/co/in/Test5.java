...
// Query 5 :- SELECT * FROM studetails WHERE S_NAME LIKE 'a%' ORDER BY ROLL_NO desc;
		...
		public static void SQLQUERY() throws Exception
		{
			...
			Query q=s.createSQLQuery("select * from studetails where S_NAME like 'a%' order by ROLL_NO desc");
		...
		public static void HQLQUERY() throws Exception
		{
			...
			Query h=s.createQuery("from StudentDTO where name like 'a%' order by rollno desc");
